...
// we evaluate that
  ...
  //    received : FE0E670000FFFF80971B01A3D600000709001F when ok
      ...
      {
          //DebugPrintln("found ecu id");
          tail = split(inMessage, ecu_id_reverse + 4);
          //DebugPrintln("tail=" + String(tail)); 
...
// the json to domoticz must be something like {"idx" : 7, "nvalue" : 0,"svalue" : "90;2975.00"}
       ...
       Serial.print(F("we publish heap to domoticz, mess is : ")); Serial.println(toMQTT);
...
// we evaluate that
    ...
    for (int x=1; x<4; x++)
    {
      ...
      DebugPrintln("hc received " + String(inMessage));
