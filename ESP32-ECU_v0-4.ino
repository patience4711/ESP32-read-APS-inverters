...
char  Mqtt_outTopic[26] = {"domoticz/in"};
char  Mqtt_Username[] = {0};
char  Mqtt_Password[] ={0};
...
// takes care for the return to the last webpage after reboot
  ...
  if( diagNose != 0 ) consoleOut("requestUrl = " + requestUrl);
...
// ******************************************************************
  ...
  unsigned long nu = millis();  // the time the program is running
   ...
   {
     Serial.println("300 secs passed, polling" + String(millis()) ); //
...
// ***************************************************************************
  ...
  if(Mqtt_Format != 0) 
  {
      if (!MQTT_Client.connected()) {
          long now = millis();
          if (now - mqtt_lastConnect > 1000UL * 10) {
...
//#endif
    ...
    if (actionFlag == 45) { //triggered by the webconsole
        ...
        testMessage(false); // the bool decides where to find the input
    }
